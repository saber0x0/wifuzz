...
def main():
    try:
        ...
        thread = subprocess.Popen(["tshark", "-w", "./capture/capture.pcap", "-i", "hci1"], stdout=devs, stderr=devs)
        ...
        try:
            userInput = (input('Chose a device number for pairing (q for quit):'))
            if userInput == 'q':
                thread.terminate()
                quit()
        ...
        try:
            serviceNum = int(input('Chose the service number :'))  # RFCOMM port
        except ValueError:
            print("Not a number")
            thread.terminate()
            quit()
